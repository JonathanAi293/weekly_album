...
export async function getLibraryAlbums(): Promise<LibraryAlbum[]> {
  const supabase = await createClient();
  const { data, error } = await supabase.from("recommendations").select("display_order, recommendation_type, recommendation_reason, review_summary, source_refs, albums(id, title, artist, cover_url, manual_cover_url, musicbrainz_release_group_id, cover_fallback_url, release_date, release_year, tags), issues(id, slug, issue_number, published_at)");
  if (error) throw error;
